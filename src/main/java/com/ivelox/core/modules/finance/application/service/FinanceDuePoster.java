...
public class FinanceDuePoster {
    ...
    public FinanceDuePoster(FinanceRepositoryPort repo) {
        this.repo = repo;
    }
    ...
    public void postForDay(LocalDate day) {
        ...
        for (var loan : repo.loans(OWNER)) {
            ...
            long remaining = remainingAll(loan.id(), loan.currency(), loan.principalMinor());
    ...
    private long remainingAll(java.util.UUID loanId, String currency, long principalMinor) {
        long paid = repo.sumLoanPayments(OWNER, loanId, currency);
