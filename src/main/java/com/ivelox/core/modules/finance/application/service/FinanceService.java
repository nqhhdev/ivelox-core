package com.ivelox.core.modules.finance.application.service;
...
import com.ivelox.core.modules.finance.api.FinanceDtos.UpcomingView;
import com.ivelox.core.modules.finance.application.port.in.FinanceUseCase;
import com.ivelox.core.modules.finance.application.port.out.FinanceRepositoryPort;
import com.ivelox.core.modules.finance.domain.model.FinanceCatalog;
import com.ivelox.core.modules.finance.domain.model.FinanceMath;
...
@Service
public class FinanceService implements FinanceUseCase {

    private final FinanceRepositoryPort repo;
    private final FinanceDuePoster duePoster;
    private final FinanceNotifier notifier;

    public FinanceService(FinanceRepositoryPort repo, FinanceDuePoster duePoster, FinanceNotifier notifier) {
        this.repo = repo;
    ...
    @Override
    public Dashboard dashboard(String userId, String monthParam, String currencyParam) {
    ...
    private List<UpcomingView> upcoming(String userId, LocalDate today, String currency) {
        LocalDate horizon = today.plusDays(7);
        List<UpcomingView> out = new ArrayList<>();
        for (var fixed : repo.fixed(userId)) {
            ...
            dueOnOrAfter(today, horizon, fixed.dayOfMonth()).ifPresent(due ->
                    out.add(new UpcomingView("fixed", fixed.name(), due, money(fixed.amountMinor(), currency))));
        }
        for (var income : repo.incomes(userId)) {
            ...
            dueOnOrAfter(today, horizon, income.dayOfMonth()).ifPresent(due ->
                    out.add(new UpcomingView("income", income.name(), due, money(income.amountMinor(), currency))));
        }
        for (var loan : repo.loans(userId)) {
            ...
            dueOnOrAfter(today, horizon, loan.dayOfMonth()).ifPresent(due ->
                    out.add(new UpcomingView("loan_payment", loan.name(), due,
        ...
        for (var saving : repo.savings(userId)) {
            ...
            dueOnOrAfter(today, horizon, saving.dayOfMonth()).ifPresent(due ->
                    out.add(new UpcomingView("saving", saving.name(), due, money(saving.monthlyAmountMinor(), currency))));
        }
        return out.stream()
    ...
    private long remainingOf(String userId, Loan loan) {
        long paid = repo.sumLoanPayments(userId, loan.id(), loan.currency());
        return Math.max(0, loan.principalMinor() - paid);
    }

    private SavingView toSavingView(String userId, SavingGoal s) {
    ...
    private LoanView toLoanView(String userId, Loan l) {
    ...
    @Override
    public TxView createTx(String userId, TxCreate req) {
        ...
        var saved = repo.insertTx(new Tx(
    ...
    public TxPage listTx(
        ...
        return new TxPage(rows.stream().map(TxView::of).toList(), nextCursor);
    }
    ...
    @Override
    public List<IncomeView> listIncomes(String userId) {
        return repo.incomes(userId).stream().map(IncomeView::of).toList();
    }

    @Override
    @Transactional
    public IncomeView createIncome(String userId, IncomeWrite req) {
    ...
    public IncomeView updateIncome(String userId, UUID id, IncomeWrite req) {
        ...
        var updated = new Income(existing.id(), existing.userId(), name, amountMinor,
                existing.currency(), dayOfMonth, existing.recurrence(), existing.createdAt());
        repo.updateIncome(updated);
        return IncomeView.of(updated);
    }
    ...
    @Override
    public List<SavingView> listSavings(String userId) {
        return repo.savings(userId).stream().map(s -> toSavingView(userId, s)).toList();
    }

    @Override
    public SavingView createSaving(String userId, SavingWrite req) {
        requireName(req.name());
        int dayOfMonth = requireDay(req.dayOfMonth());
        var settings = repo.settings(userId);
        String currency = req.currency() == null || req.currency().isBlank()
        ...
        var saved = repo.insertSaving(new SavingGoal(
    ...
    @Override
    public SavingView updateSaving(String userId, UUID id, SavingWrite req) {
        ...
        var updated = new SavingGoal(existing.id(), existing.userId(), name, targetMinor,
    ...
    @Override
    public List<LoanView> listLoans(String userId) {
        return repo.loans(userId).stream().map(l -> toLoanView(userId, l)).toList();
    }

    @Override
    @Transactional
    public LoanView createLoan(String userId, LoanWrite req) {
        requireName(req.name());
        int dayOfMonth = requireDay(req.dayOfMonth());
        var settings = repo.settings(userId);
        String currency = req.currency() == null || req.currency().isBlank()
        ...
        var saved = repo.insertLoan(new Loan(
    ...
    @Override
    public LoanView updateLoan(String userId, UUID id, LoanWrite req) {
        ...
        var updated = new Loan(existing.id(), existing.userId(), name, principalMinor,
    ...
    @Override
    public List<FixedView> listFixed(String userId) {
        return repo.fixed(userId).stream().map(FixedView::of).toList();
    }

    @Override
    public FixedView createFixed(String userId, FixedWrite req) {
        requireName(req.name());
        int dayOfMonth = requireDay(req.dayOfMonth());
        var settings = repo.settings(userId);
        String currency = req.currency() == null || req.currency().isBlank()
    ...
    public SettingsView getSettings(String userId) {
        ...
        return new SettingsView(s.homeCurrency(), s.digestHour(), s.digestEnabled(),
                s.overSlotAlert(), s.overMonthAlert());
    }

    @Override
    public SettingsView updateSettings(String userId, SettingsWrite req) {
        ...
        var updated = new Settings(userId, homeCurrency, digestHour, digestEnabled, overSlot, overMonth);
        repo.saveSettings(updated);
        return new SettingsView(updated.homeCurrency(), updated.digestHour(),
